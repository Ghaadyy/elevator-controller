 ...
 * @date		05/10/22
 ...
 mailbox s_task 3-level priority added 1.0.4	eTicks added to sTask to count
 elapsed ticks Semaphore handling added (5.084) 1.0.3	InitScheduler function
 added s_task_idle sTask added to GetSchedulerStats functionBusy LED option
 added (5.084) 1.0.2	Added support to NIOS2 (15.1) 1.0.1	Functions
 modified: sTaskCreate(..) sTaskResume(..) sTaskSetPeriodicTicks(..) Scheduler()
...
#ifdef USE_RETIMER
#if (defined(__PCH__) || defined(__PCD__)) || defined(__NIOS2__) || defined(STM32F40XX) || defined(STM32F4XX) ||       \
    defined(STM32F2XX) || defined(__XC16__)
...
const s_task_priority_t priority_cycle[PRIORITIES_CYCLE_LEN] = {S_TASK_HIGH_PRIORITY, S_TASK_NORMAL_PRIORITY,
                ...
                if (cs_task->active) /*is it active?*/
                {
                    if (cs_task->s_task != NULL) /*task's function exists?*/
                    {
                        if (priority_cycle[prior_cycle_index] == cs_task->s_priority) /*check priority*/
                        {
                            ...
                            if ((ticks_diff >= cs_task->s_ticks) ||
                                (S_TASK_ASYNC_MODE == cs_task->s_mode)) /*should run?*/
                            {
                                ...
                                (*cs_task->s_task)(cs_task->s_task_id, &(cs_task->s_task_msg),
                                                   (cs_task->a_task_arg)); /*run task*/
 ...
 * @return false: fail, true: success
 */
bool s_task_create(bool active, s_task_priority_t s_task_prio, cntr_size_t s_ticks, s_task_t s_task_ptr,
                   s_task_handle_t *s_tsk_hndl_ptr, void *s_arg)
{
 ...
 * @return false: fail, true: success
 */
bool s_task_send_msg(s_task_handle_t s_task_src_hndl, s_task_handle_t s_task_dst_hndl, uint8_t *msg_bytes,
